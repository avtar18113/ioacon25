...
function fetchRegistrationData1($conn) {
    $query = "SELECT email FROM registration WHERE p_status='success' AND email != '' AND sendmail=0";
...
function fetchRegistrationData($conn,$email) {
    $query = "SELECT * FROM registration WHERE p_status='success' AND email= '$email' AND sendmail=1";
...
$result1 = fetchRegistrationData1($conn);
...
if (mysqli_num_rows($result1) > 0) {
    ...
    echo "<script>alert('No records found to send emails.'); window.location.href='$regPath/backend_home.php';</script>";
}
